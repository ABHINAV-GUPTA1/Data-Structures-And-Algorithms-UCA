...
abstract class MyAbstractClass {
	...
	abstract void bar();
//	void abc(); <-- Error, must include "abstract" keyword, or otherwise define the function
	public void bax() {
		System.out.println("Method: bax()");
	}
}
...
//	void foo() { System.out.println("Method: foo()"); }	<-- Error, as function definition is not allowed in interfaces
				// Note here we are considering Java7 but acc. to java 8 we can define function in interface itself.
	void bar();
	abstract void baz();
//	private void abc();	<-- Error, interface methods are public and abstract only
}
...
class MyClass extends MyAbstractClass implements MyInterface, MyAnotherInterface {
	public void bar() { 
		System.out.println("Abstract Method: bar()"); 	// Must be public, as interface methods are public and we can not downcast
	}
	public void baz() { 
		System.out.println("Abstract Method: baz()"); 	// Must be public, as interface methods are public and we can not downcast
	}
}
/* Main class */
public class AbstractClassAndInterface {
	...
	//	mi = new MyInterface();		<-- Error, can not directly instantiate
		...
		mc.baz();
		mc.bax();
	//	System.out.println(mc.y + " " + MyClass.z);	<-- Error, when multiple 
	//  variables of same name are inherited, it is ambiguous to refer to them
	}
}
